...
function thief_game_loop() {
    ...
    map.on('style.load', function () {
        ...
        thief.initialize(new mapboxgl.LngLat(-122.265491, 37.796931), new mapboxgl.LngLat(-122.42, 37.78));
        thief.get_directions(maputils, thief_directions_updated);
    });
}
...
function cop_game_loop() {
    ...
    socket.on('thief_loc', function(loc) {
         if (thief == null) {
             thief = new Actor(THIEF, 0);
             thief.startpoint = mapboxgl.LngLat(loc["lng"], loc["lat"]);
             thief.add_icon(maputils);
        }

         var pos = mapboxgl.LngLat(loc["lng"], loc["lat"]);
    ...
    socket.on('cop_loc', function(user_data) {
        ...
        if (!(id in cop_markers)) {
            ...
            cop_markers[id].currentpos = mapboxgl.LngLat(lng, lat);
    ...
    function initialize_cop(thief_loc) {
        ...
        me.initialize(mapboxgl.LngLat(-122.265491, 37.796931), mapboxgl.LngLat(thief_loc["lng"], thief_loc["lat"]));
        me.get_directions(maputils, my_directions_updated);
    }
    
    map.on('click', function(e) {
        window.clearTimeout(moveStep);
        me.endpoint = mapboxgl.LngLat(e['lngLat']['lng'], e['lngLat']['lat']);
        me.reset();
        // maputils.clearPath();
        me.get_directions(maputils, my_directions_updated);
    });
